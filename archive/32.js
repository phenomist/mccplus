...
var mccPlus_Version            = "31.5";
var mccPlus_Version_Date       = "05/19/2015 21:51 PST";
var mccPlus_RBUp = "";
var mccPlus_Raid = "";
var mccPlus_GBUp = "";
...
document.getElementById("vault_container").innerHTML='<center><div id="raid1c"></div></center><div id = "employment2"><table name="soldiers2" cellpadding="8" type="tab" class="hidden" style="display: table;"><tbody><tr name="knight"><td><img src="game/img/npc/viking.png" height="32"><br><b>Viking</b><br>$<span name="price">300</span> - KPE: <span name="kpe">4</span></td><td><button name="buy-knight">Buy</button><button name="buyx-knight">Buy X</button><br><button name="buymax-knight">Buy Max</button><button name="sellx-knight">Sell X</button><br>You own <span name="owned"></span><br>Total KPE: <span name="totalKPE"></span></td></tr></table><table><tr><td><img src="game/img/npc/steve.png" height="32"><br><b><span name="scientists_price">2 BC</span></b><br>Rate: +<span name="scientistTime">15</span>s<br><br><center><font size="2"><a href="#" name="scientistBuyMode" style="color: rgb(42, 110, 116);">Change currency type to money</a></font></center></td><td>You own <span name="scientists_owned">87830</span> scientists.<br><button name="hire_scientist">Buy</button> &nbsp;<button name="fire_scientist">Sell</button><br><button name="hirex_scientists">Buy X</button><button name="hiremax_scientists">Buy max</button></td></tr></table></div>'
...
// v32: Shortcut boxes
// Making things work
		$(document).on('click', '#employment2 button[name="hire_scientist"]', buyScientist);
		$(document).on('click', '#employment2 button[name="hiremax_scientists"]', buyMaxScientists);
		$(document).on('click', '#employment2 button[name="hirex_scientists"]', buyXScientists);
		$(document).on('click', '#employment2 button[name="fire_scientist"]', sellScientist);
		$(document).on('click', '#employment2 a[name="scientistBuyMode"]', function(e){
			e.preventDefault();
			client.emit('toggleBuyMode', true);
		});
    		$(document).on('click', 'table[name="soldiers2"] button[name|="buy"]', function() {
			buySoldier($(this).attr('name').split('-')[1], false)
		});
		$(document).on('click', 'table[name="soldiers2"] button[name|="sell"]', function() {
			sellSoldier($(this), false)
		});
		$(document).on('click', 'table[name="soldiers2"] button[name|="buyx"]', function() {
		...
		$(document).on('click', 'table[name="soldiers2"] button[name|="buymax"]', function() {
			var soldierType = $(this).attr('name').split('-')[1];
			var amount = Math.floor(game.money/items.soldiers[soldierType].price);
			buySoldier(soldierType, amount);
		});
		$(document).on('click', 'table[name="soldiers2"] button[name|="sellx"]', function() {
			sellSoldier($(this), true)
		});

		function buyScientist()
		{
			mccPlusClient.emit('buyScientist', true);
		}
					...
					'func': function() {
						...
						if( !game.scientistBCMode )
						{
							...
							if( game.money >= totalcost )
								mccPlusClient.emit('buyXScientists', amount);
							else
		...
		function buyMaxScientists()
		{
			if( !game.scientistBCMode )
			{
				...
				if( game.money >= totalcost )
				{
					mccPlusClient.emit('buyMaxScientists', true);
				}
			}
			else
			{
		...
		{
			mccPlusClient.emit('sellScientist', true);
		}
    		...
    		function buySoldier(soldierType, x)
		{
			...
			{
				if( amount == 1 )
					mccPlusClient.emit('buySoldier', {soldierType: soldierType, amount:1});
				else if( amount > 1 )
				{
					mccPlusClient.emit('buySoldier', {soldierType: soldierType, amount:amount});
				}
			}
		}
		...
		function sellSoldier(obj, x)
		{
			...
			if( game.employedSoldiers[soldierType] >= amount )
			{
				if( amount == 1 || (amount > 0 && confirm('Are you sure you wish to sell ' + amount + ' ' + name + 's?')) )
					mccPlusClient.emit('sellSoldier',  {soldierType: soldierType, amount:amount});
			}
		}

// v33: Compact inventory
