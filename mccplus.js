...
(function() {
	// We inject mccp in a script tag; this should be the 'native-st' way of running it and it plays nice with userscripts and whatnot.
	function mccpRoot() {
		...
		mccp.socketOn = function(ev, handler) {
			this.client.on(ev, handler);

			this.unloadFuncs.push(function() {
				this.client.off(ev, handler);
			}.bind(this));
		...
		mccp.unload = function() {
			console.log('Unloading MCC+');
			this.unloadFuncs.forEach(function(fn) {
				fn();
			});
			$('.mccp-component').remove();
			mccp.loaded = false;
			console.log('Unloaded MCC+');
		};

		mccp.load = function() {
			console.log('Loading MCC+');
			this.loaded = true;
			...
			this.socketOn('loginAttemptResult', function(data) {
				if (! data.success) return;
				mccPlus_Username = data.username;
			});
			...
			var GBStats_CBot = function(channel, data) {
				if (mccPlus_Username === 'Guest') return;
					...
					if (cieq((args[0] || ''), mccPlus_Username) || (cieq(mccPlus_Username, requester) && args[0] == undefined)) {
						switch (command.toLowerCase()) {
							...
							case "version":
								...
								break;
							case "link":
								text = ".\nNormal script: https://cdn.rawgit.com/phenomist/mccplus/master/mccplus.js\nUserscript: https://cdn.rawgit.com/phenomist/mccplus/master/mccplus.user.js\nRepository: https://github.com/phenomist/mccplus";
								break;
						}
	...
	script.innerHTML = '(' + mccpRoot.toString() + ')();';
